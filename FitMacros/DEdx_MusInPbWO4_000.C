TSplineFit* DEdx_MusInPbWO4_000(Bool_t todraw = kFALSE,Bool_t infile = kFALSE,Bool_t firstinfile = kFALSE)
//
// Arguments:
//
...
//  DEdx_MusInPbWO4_000
//
//  Landau distribution of dE/dx of muons of less than 10 Gev in PbWO4.
//  Values in Mev
...
//
{
  ...
  Double_t s17[M];
  for (i=0;i<M;i++) s17[i] = TMath::Sqrt(y[i]);
  DEdx0 = new TSplineFit("DEdx_MusInPbWO4","Energy deposited by crossing | Muons in PbWO4",
    11,M,m,x,y,s17,kTRUE,0.0,kFALSE,100000.0,0.0,60.0,kFALSE);
  DEdx0->BelongsToFamily(0,5.0,"Energy of muon");
  DEdx0->SetSource("Calculated with GEANT by A.Givernaud DSM/DAPNIS/SPP CEA Saclay");
  DEdx0->SetMacro("DEdx_MusInPbWO4_000.C");
  DEdx0->SetXLabel("deposited energy [Mev]");
  DEdx0->SetVLabel("Nb. of hits according to GEANT 3");
  DEdx0->UseForRandom();
  if (todraw) {
    DEdx0->SetDefaultLabels();
    DEdx0->DrawFit();
    DEdx0->Print();
  }
  if (infile) {
    if (firstinfile) DEdx0->UpdateFile(kTRUE);
    else             DEdx0->UpdateFile(kFALSE);
  }
  return DEdx0;
}
