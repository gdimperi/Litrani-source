{
//
//  CMSEcalEC_setup.C is a description of the final setup for an End-Cap crystal of the 
//CMS Ecal detector. The material PbWO4 of the crystal is treated as birefringent.
//On the upper face of the PbWO4 crystal is the APD capsule AND the optical fibre
//(contrary to the Barrel crystals which have the fibre on the opposit small end
//face of the crystal). This  requires this upper face to be divided into:
//  - face 5,    the part neither covered by the APD capsule nor by the fibre support
//  - subface 6, the part covered by the APD capsule. The 2 APDs are not directly in
//               contact with the crystals. They are glued. We will describe this glue
//               as 2 TSBRIK, one per APD.
//  - subface 7, the part covered by the fibre support. The fibre is not in contact
//               with the crystal. There is a space of air between fibre and crystal
//               we will describe this space of air as a TSBRIK.
//  So there are 7 shapes in this setup :
//
// - (a) 1 TSTRAP : CMS crystal of type 18 (Endcap) made of PbWO4
// - (c) 2 TSBRIK : glue of the 2 APDs
// - (d) 2 TSBRIK : the 2 APDs made of silicium
...
//2 APDs and the fibre. The part neither covered by the glue of the APDs nor by the
...
//Between the glue and the silicium of the APDs, there is a thin slice of nitrure which
//is the window of the APD. This is a true thin slice in the optical meaning of the word,
//i.e. a slice which is so thin that its thickness is of the order of the wavelengths,
//so that the normal Fresnel formulae do not apply. Litrani knows how to handle optical
//thin slices.
//
...
//Silicium is isotropic with index of refraction depending  upon wavelength
//according to the fit "Refr_I_Silicium".
...
//seems that in fact the revetment is essentially black, which explains why
//we do not use "RIndexRev_CMSalveola", "IIndexRev_CMSalveola".
...
//
//  Definition of the wrapping "Revtop", describing the surface
...
//
  ...
  crystal->GetCMSdim(crystaltype,ferule_BF,crys_AF,crys_BF,crys_CF,crys_AR,crys_BR,
  	crys_CR,iso_dx);
  if (verbosity >= 2) then {
    cout << endl;
...
//
//  We create a subface on face 5 of crystal, which is the portion of the face which is
//in contact with the air gap of the fibre.
  ...
  if (verbosity >= 3) {
    cout << "  Rectangle of the support of the fibre :" << endl;
    cout << "  p12 : "; p12->Print();
    cout << "  p13 : "; p13->Print();
    cout << "  p14 : "; p14->Print();
    cout << "  p15 : "; p15->Print();
  ...
  ps->AddSubPoint(p15);
  TFace *face7;
  face7 = new TFace(crystal,7,5,polygonal,ps->fPoints,ps->fGCenter,"none",
  	12,13,14,15,kFALSE);
  ps->AddSubFace(face7);
...
//We define the center of face 5.
  ...
  if (verbosity >= 2) cout << "  center  face 5 : (" << center5_x << ',' << center5_y << ')' << endl;
...
//Position in space
  ...
  TContact *CrystalGlue1,*GlueAPD1,*CrystalGlue2,*GlueAPD2,*CrystalAir,*AirFibre;
  CrystalGlue1 = new TContact("crystal_glue1","crystal_glue1","Crystal","glue1",containing);
  GlueAPD1     = new TContact("glue1_apd1","glue1_apd1","glue1","apd1",identical);
  CrystalGlue2 = new TContact("crystal_glue2","crystal_glue2","Crystal","glue2",containing);
  GlueAPD2     = new TContact("glue2_apd2","glue2_apd2","glue2","apd2",identical);
