// @(#)root/html:$Id: TSTRD2.cpp 2008-06-11
...
//
// TSTRD2 inherits from TTRD2 and adds supplementary information required for
...
//    Double_t   dy1        half-length along y at the z surface positioned at -DZ in cm
//    Double_t   dy2        half-length along y at the z surface positioned at +DZ in cm
...
//
//
// The 8 points of a TSTRD2 are defined in the following way :
//
//  point[0] = (-dx1,-dy1,-dz)
//  point[1] = (+dx1,-dy1,-dz)
//  point[2] = (+dx1,+dy1,-dz)
...
//  point[7] = (-dx2,+dy2,+dz)
//
// The 6 faces of the TSTRD2 are built from these points in the following way :
//
...
//shapes. This associated class may be reached from TSTRD2 (or from any
//other shape) like this : (supposing that ptrd2 is a pointer on a TSTRD2)
//
//     TSupplShape *suppl = ptrd2->fSuppl;
//
...
//
TSTRD2::TSTRD2(const char *name,const char *title,const char *namemat,
  const char *namerev,Double_t dx1,Double_t dx2,Double_t dy1,Double_t dy2,
  Double_t dz,Bool_t FromOther):TTRD2(name,title,namemat,Float_t(dx1),
  Float_t(dx2),Float_t(dy1),Float_t(dy2),Float_t(dz)) {
  //constructor
  ...
  if (FromOther) {
    nameshape = namemat;
    fSuppl = new TSupplShape(name,title,this,tstrd2,nameshape,namerev);
  }
  else {
    fSuppl = new TSupplShape(name,title,this,tstrd2,namemat,namerev);
  }
...
void TSTRD2::SetOpticalAxis(Double_t thetax, Double_t phix, Double_t thetay, Double_t phiy) const {
